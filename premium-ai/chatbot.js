...
// Fungsi untuk mengenkripsi teks dengan metode substitusi
function encrypt(text) {
  return btoa(text);
}

// Kode JavaScript yang akan dienkripsi
const code = `
...
function verifyPassword(enteredPassword) {
  const encryptedPassword = "${encrypt("mejikuhibiniudisiniuserpremium2024byzukuaibwahahahah")}";
  return enteredPassword === atob(encryptedPassword);
}

// Main program
const enteredPassword = promptPassword();
if (verifyPassword(enteredPassword)) {
  alert("Password benar. Akses diizinkan.");
} else {
...
// Enkripsi kode JavaScript
const encryptedCode = encrypt(code);

// Buat elemen <script> baru dengan kode JavaScript yang dienkripsi
const scriptElement = document.createElement("script");
scriptElement.textContent = `eval(atob("${encryptedCode}"));`;
